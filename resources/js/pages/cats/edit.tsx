...
export default function EditCat() {
    const { cat } = usePage<PageProps>().props;
    ...
    const [previewAvatar, setPreviewAvatar] = useState<string | null>(cat.avatar);
            ...
            <div className="flex flex-col w-full min-h-screen bg-[#EAF2F9] pb-24">
                ...
                <div className="bg-gradient-to-br from-blue-400 via-indigo-500 to-purple-600 px-4 pt-6 pb-8 shadow-lg">
                    <div className="flex items-center gap-3">
                        <button
                            onClick={() => router.visit(`/cats/${cat.id}`)}
                            className="w-10 h-10 bg-white/20 backdrop-blur-md rounded-full flex items-center justify-center text-white hover:bg-white/30 transition-colors"
                        >
                            <ArrowLeft className="w-5 h-5" />
                        </button>
                        <h1 className="text-xl font-bold text-white">Edit {cat.name}</h1>
                    </div>
                </div>
